...
export async function buildMemoryContext(
  ...
  // to avoid V8 memory reallocation overhead in dynamic loops
  const lines: string[] = [`[Mnemo Context for "${projectName}"]`]
  let usedBudget = lines[0].length + 1 // +1 for the \n

  for (const mem of results) {
    const lineContent = `- [${mem.category}] ${mem.content}`
    const lineLen = lineContent.length + 1 // +1 for \n

    if (usedBudget + lineLen > budget) {
      // Try to fit a truncated version
      const remaining = budget - usedBudget
      if (remaining > 20) {
        // We append the raw string with \n to truncate properly as original did
        const truncated = truncateToFit(`${lineContent}\n`, remaining)
        lines.push(truncated.endsWith('\n') ? truncated.slice(0, -1) : truncated)
      }
      break
    }
...
export async function exportMemoriesAsMarkdown(bridge: MnemoBridge): Promise<string> {
  ...
  // Optimization: Use string array + join instead of string concatenation (+=)
  // to avoid V8 memory reallocation overhead in dynamic loops
  const lines: string[] = ['# Mnemo Memory\n', `> Auto-generated by mnemo-plugin. ${total} memories found.\n`]
  ...
  for (const [category, mems] of grouped) {
    ...
    lines.push('') // Add empty string for newline separation
  }
