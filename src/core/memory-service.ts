...
export async function buildMemoryContext(
  ...
  const header = `[Mnemo Context for "${projectName}"]\n`
  let usedBudget = header.length
  ...
  for (const mem of results) {
    ...
    if (usedBudget + line.length > budget) {
      ...
      if (remaining > 20) {
        lines.push(truncateToFit(line, remaining))
      }
      break
    }
    lines.push(line)
